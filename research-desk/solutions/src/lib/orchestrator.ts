...
class DeskOrchestrator {
  ...
  private async handleDispatch(client: Anthropic, headSessionId: string, event: SessionEventLike): Promise<void> {
    ...
    // The head's session is now idle at requires_action — the platform is
    // waiting on THIS SERVER to answer. Run the fan-out, then post the result
    // back tied to the tool-use id so the head can resume.
    let resultPayload: string;
    try {
      if (tickers.length === 0) {
        resultPayload = JSON.stringify({ scorecards: [], failures: [{ ticker: "", error: "no tickers provided" }] });
      } else {
        ...
        resultPayload = compileDispatchResult(dispatch.records);
      }
      dispatch.status = "completed";
    } catch (error) {
      dispatch.status = "failed";
      dispatch.error = error instanceof Error ? error.message : String(error);
      resultPayload = JSON.stringify({ scorecards: [], failures: [{ ticker: "", error: dispatch.error }] });
      logEvent("error", "dispatch_failed", { tool_use_id: toolUseId, error: dispatch.error });
    } finally {
    ...
    logEvent("info", "dispatch_answered", { tool_use_id: toolUseId, status: dispatch.status });
  }
